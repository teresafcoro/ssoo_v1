...
int OperatingSystem_LongTermScheduler() {  
	...
	for (i=0; programList[i]!=NULL && i<PROGRAMSMAXNUMBER ; i++) {
		...
		if (PID == NOFREEENTRY)	// V1_EJERCICIO_4
			ComputerSystem_DebugMessage(103, ERROR, "There are not free entries in the process table for the program", programList[i]->executableName);
...
void OperatingSystem_PCBInitialization(int PID, int initialPhysicalAddress, int processSize, int priority, int processPLIndex) {
	...
	// V1_EJERCICIO_10
	ComputerSystem_DebugMessage(111, SYSPROC, PID, programList[processPLIndex]->executableName);
...
void OperatingSystem_TerminateProcess() {  
	...
	// V1_EJERCICIO_10
	ComputerSystem_DebugMessage(110, SYSPROC, executingProcessID, programList[processTable[executingProcessID].programListIndex]->executableName, statesNames[processTable[executingProcessID].state], statesNames[4]);
	...
	switch (systemCallID) {
		...
		case SYSCALL_YIELD:
			...
			if (processTable[previousPID].priority == processTable[readyToRunQueue[cola][0].info].priority) {
				ComputerSystem_DebugMessage(115, SHORTTERMSCHEDULE, previousPID, programList[processTable[previousPID].programListIndex]->executableName, readyToRunQueue[cola][0].info, programList[processTable[readyToRunQueue[cola][0].info].programListIndex]->executableName);
